...
| `PLATFORM_ADMIN_TELEGRAM_IDS`        | api           | Telegram ID администраторов платформы через запятую   |
...
## Регистрация, вход и подписка

- **Владелец бизнеса** регистрируется в Mini App: бренд → email → пароль. Компания получает статус
  «Заявка» и ждёт активации. Вход владельца — email + пароль.
- **Сотрудник** входит по названию магазина (бренду), логину и паролю — их задаёт владелец
  в «Ещё → Сотрудники».
- После первого входа по паролю внутри Telegram аккаунт привязывается — дальше Mini App
  открывается сразу.
- **Админ-панель платформы** (владелец MyShop): «Ещё → Админ-панель» в Telegram, доступ по Telegram ID
  из `PLATFORM_ADMIN_TELEGRAM_IDS`. Список компаний, активация, продление на 1 / 3 / 12 месяцев,
  блокировка.
- **Подписка:** активна до даты оплаты; после — только просмотр (изменения → `SUBSCRIPTION_EXPIRED`);
  заявка и блокировка — работать нельзя. Пароли хранятся только как scrypt-хэш.
