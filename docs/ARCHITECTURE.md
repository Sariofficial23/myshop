...
- Этап 4: остаток = `StockBalance` (агрегат) + неизменяемая история `StockMovement`.
  Меняется только через `StockService.applyMovement` внутри транзакции документа:
  строка остатка блокируется (`SELECT … FOR UPDATE`), отрицательный остаток запрещён
  кодом (`INSUFFICIENT_STOCK`) и CHECK-ограничением БД, себестоимость — средневзвешенная.
  Документы проводятся условным переходом `DRAFT → CONFIRMED`, поэтому провести дважды нельзя.
- Этап 5: `Sale` → `SaleItem` (цена, скидка, сумма, себестоимость на момент продажи) → `Payment`.
  Продажа списывает остаток через тот же `StockService.applyMovement` (тип `SALE`), а IMEI
  переводится `IN_STOCK → SOLD` условным `updateMany` в той же транзакции: если параллельная
  продажа уже забрала номер, вся продажа откатывается с `IMEI_ALREADY_SOLD`.
- Этап 6: `SaleReturn` (+ `ReturnItem`, `Refund`), `Transfer`, `WriteOff`, `Inventory` — у каждого
  движения `StockMovement` есть ссылка на документ-источник. Возврат/перемещение/списание проводятся
  сразу, инвентаризация — черновик → проведение. Общие проверки строк — `common/documents/stock-lines.ts`.
