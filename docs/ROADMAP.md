...
| 7    | Customers, Suppliers, Cash, Installments, Warranties                                  | ✅ Готово |
...
- Рассрочка: продажа с первоначальным взносом (можно 0), нужен клиент (`CUSTOMER_REQUIRED`);
  график платежей (ежемесячный платёж с округлением вверх), просрочка на сегодня, приём платежей
  с блокировкой строки (переплата невозможна — `INSTALLMENT_OVERPAYMENT`), статус PAID;
  возврат товара сначала уменьшает долг, деньгами выплачивается только остаток
- Касса филиала: остаток наличных выводится из документов (продажи и рассрочки наличными −
  возвраты + внесения − изъятия − расходы), итоги дня по часовому поясу компании; изъятие/расход
  больше остатка запрещены (`CASH_INSUFFICIENT`, advisory lock)
- Гарантия: проверка по IMEI (чек, клиент, срок), обращения ГР-n со статусами
  принят → в ремонте → готов → выдан / отказ; вне гарантии — платный ремонт
- Карточки клиента (покупки, сумма, долг, просрочка) и поставщика (приходы, сумма)
- Отчёт о продажах `GET /reports/sales` (перенесён вперёд по запросу): сумма, чеки, товары,
  средний чек, прибыль, возвраты, деньги по способам оплаты, что продано, продавцы
- UI в стиле iOS: шрифт SF Pro / Inter, системный синий, иконки lucide в цветных плитках,
  нижняя вкладка «Отчёты», карточка «Сегодня» на главной
